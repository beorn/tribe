...
| `TRIBE_LLM_DIR`                                        | recall, injection  | `@bearly/llm` source directory exposing its public `index.ts` barrel, including query, provider-fact/store, selector, and dispatch-failure APIs. Enables LLM synthesis, the query planner, and session summaries. Without it—or when required exports are missing—those features degrade to their documented no-LLM paths loudly and name the missing capabilities. |
