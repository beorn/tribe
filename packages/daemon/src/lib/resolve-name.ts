...
/** True for the daemon's placeholder name for an unidentified session. */
export function isUnidentifiedSessionName(name: string): boolean {
  return /^unknown-[a-z0-9]{3,}$/.test(name)
}
...
export function isAutoGeneratedName(name: string): boolean {
  ...
  if (isUnidentifiedSessionName(name)) return true
