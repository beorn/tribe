 ...
 *
 * `sequence` selects the fact identity. Vary it to exercise a THRESHOLD:
 * `evaluateAlerts` deliberately skips re-judging numbers it has already judged,
 * so a threshold fixture that reused one sequence would measure the skip rather
 * than the reading.
 *
 * RE-PINNED, and the correction is the point. This comment used to say a reused
 * sequence tests "the skip rather than the reading" FULL STOP, which described
 * the defect `@ci` returned head 6dee0742 for as though it were the contract.
 * The blindness clearing edge is NOT a threshold evaluation and no longer sits
 * behind that gate, so a repeated identity is still a reading — see "clears on
 * a journal RETRY that repeats the PRE-OUTAGE fact identity", which reuses one
 * sequence ON PURPOSE and would be impossible to write under the old sentence.
 */
...
describe("a monitor that cannot measure the disk says so (@i/4-supervision/24233)", () => {
  ...
  it("clears on a journal RETRY that repeats the PRE-OUTAGE fact identity", () => {
    // RETURNED UNADMITTED by `@ci` at head 6dee0742 for exactly this. The
    // recovery clearing lived inside the `evaluateScalarMetrics` dedupe gate,
    // which exists to stop the evaluator re-judging the same numbers — right
    // for thresholds, wrong for the blindness clearing edge.
    //
    // The mechanism, and it needs no exotic input: while blind, the observation
    // carries NO identity, so `state.lastScalarFact` is never overwritten and
    // still holds the last PRE-OUTAGE epoch+sequence. A journal retry that
    // re-emits that same fact therefore reads as "same fact, skip" — the clear
    // never runs, `disk:blind` stays latched, and the NEXT real outage counts
    // up to two and is swallowed by its own dedupe. That is the swallowed-alert
    // failure this whole change exists to end, reached through the dedupe
    // instead of through the counter.
    //
    // Every other recovery case on this page starts from a fresh state, so its
    // recovery sample is a FIRST sighting and the gate lets it through. That is
    // why they are all green against this defect.
    const state = createAlertState()
    const thresholds = defaultThresholds()

    // The pre-outage reading — the part the other cases lack. It leaves
    // `lastScalarFact` holding this identity, so the retry below is a REPEAT.
    evaluateAlerts(seeingMetrics(0.1, 7), thresholds, state)

    evaluateAlerts(blindMetrics(), thresholds, state)
    expect(blindAlerts(evaluateAlerts(blindMetrics(), thresholds, state))).toHaveLength(1)

    // THE RETRY: same epoch, same sequence as before the outage.
    evaluateAlerts(seeingMetrics(0.1, 7), thresholds, state)
    expect(state.firedAlerts.has("disk:blind"), "a repeated fact identity is still a READING").toBe(false)
    expect(state.scalarBlindSamples, "the counter must reset on any sample that can see").toBe(0)

    // The consequence, which is the one that reaches an operator: a second
    // outage must still page rather than be swallowed by a stale dedupe entry.
    evaluateAlerts(blindMetrics(), thresholds, state)
    expect(
      blindAlerts(evaluateAlerts(blindMetrics(), thresholds, state)),
      "the second outage is swallowed when the retry failed to clear the dedupe entry",
    ).toHaveLength(1)
  })
...
describe("END TO END through the consumer's own read path (@cto term 3b)", () => {
  it("a contradictory hab environment reaches the ALERT, not a silent standalone", async () => {
    ...
    // MEASURED BOUND, not a guess, and not vitest's default. This case is end
    // to end on purpose, so it pays the real cost of `collectFullMetrics`:
    // `ps`, a worktree listing, an fd count and `ulimit`, twice. Measured on
    // this host 2026-09-07: 2676ms and 2689ms for the two calls — 5.4s against
    // a 5000ms default, so the case was marginal from the day it was written
    // and failed the moment the host got busier. The cost scales with the
    // machine's process and worktree counts, both of which only grow, so the
