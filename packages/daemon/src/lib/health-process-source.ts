...
export interface HealthProcessSourceOptions {
  ...
  readonly now?: () => number
  /** A fixed ceiling for every snapshot; unset, each snapshot's ceiling scales with the load (sysmonCommandTimeoutMs). */
  readonly commandTimeoutMs?: number
  /** The one-minute load average, read as each snapshot spawns. Defaults to os.loadavg()[0]. */
  readonly loadAverage?: () => number
  /** The cores the load is shared by. Defaults to os.availableParallelism(). */
...
function managedProcessSource(
  ...
  const loadAverage = options.loadAverage ?? (() => loadavg()[0] ?? 0)
  const cores = options.cores ?? availableParallelism()
  const ceilingMs = (): number => options.commandTimeoutMs ?? sysmonCommandTimeoutMs(loadAverage(), cores)
