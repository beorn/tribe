...
>(hooks: DispatcherRuntimeHooks = {}): (t: T) => T & WithDispatcher {
  return (t) => {
    ...
    /**
     * Secure-inbox access authority (successor r2) — draining/reading a name's
     * inbox advances its durable actionable cursor (acknowledging, i.e. hiding,
     * mail), so it needs the same authority as attach. Authorized when the
     * caller connection IS the live holder of the name, OR presents operator
     * authority, OR presents credentials matching the target's STORED identity.
     * A name with no bound durable row (e.g. an unregistered `@chief` host
     * mailbox) stays open — the same migration path as attach.
     */
    function authorizeInboxAccess(
      name: string,
      connId: string,
      p: Record<string, unknown>,
    ): { ok: true } | { ok: false; message: string } {
      const caller = clients.get(connId)
      if (caller && caller.role === "member" && caller.name === name) return { ok: true }
      if (operatorAuthorized(p)) return { ok: true }
      const row = isAutoGeneratedName(name)
        ? null
        : (stmts.getDurableSessionByName.get({ $name: name }) as DurableIdentityRow | null)
      if (!row || !rowHasStoredCredential(row)) return { ok: true }
      if (presentedMatchesStored(row, parsePresentedIdentity(p))) return { ok: true }
      return {
        ok: false,
        message: `inbox drain for "${name}" denied: the name is bound to a stored identity and the caller is not its authenticated holder`,
      }
    }
      ...
      try {
        switch (method) {
          case "register": {
            ...
            // still holding this name is DETACHED. NAME IS A LOCATOR, NOT AN
            // AUTHENTICATOR: before a claimant may inherit that row's sessionId
            // (its recipient mailbox + pending balls) OR evict it to squat the
            // name, the row's STORED credential must authorize the claim. A row
            // with no stored credential (legacy/pure-CLI) is claimable by name —
            // the documented migration path. Auto-generated names (codex1,
            // unknown-x, member-x) are re-minted per spawn and carry no durable
            // identity, so they are never gated here. See identity-authority.ts.
            const claimRow = isAutoGeneratedName(name)
              ? null
              : (stmts.getDurableSessionByName.get({ $name: name }) as DurableIdentityRow | null)
            const presentedIdentity: PresentedIdentity = { identityToken, launchIdentity }
          ...
          case "cli_leave": {
            ...
            // Authority (successor r2): only the authenticated holder connection
            // (self) or operator authority may leave/destroy a named member. An
            // arbitrary connection can no longer retire a live holder and delete
            // its durable row — the r1 unowned-destructive-access hole.
            const leaveCaller = clients.get(connId)
            const leaveIsSelf = !!leaveCaller && leaveCaller.role === "member" && leaveCaller.name === sessionName
          ...
          case "cli_inbox_drain": {
            ...
            // Secure-inbox authority (successor r2): a drain advances the durable
            // actionable cursor — acknowledging (hiding) mail — so it requires the
            // holder connection, operator authority, or matching stored identity.
            // Arbitrary-name drain of a BOUND member is denied and the victim's
            // cursor is left untouched.
            const drainAuthz = authorizeInboxAccess(sessionName, connId, p)
            if (!drainAuthz.ok) {
              return makeError(id, TRIBE_IDENTITY_UNAUTHORIZED, drainAuthz.message, {
