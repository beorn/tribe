      ...
      try {
        switch (method) {
          case "register": {
            ...
            // identity_token + the launch tuple are LEGACY LOCATION HINTS only —
            // never authority. The sole authenticator is the server-minted
            // `capability` a client presents (received on its first register).
            const identityToken = (p.identityToken as string) ?? null
            const presentedCapability =
              typeof p.capability === "string" && p.capability.length > 0 ? p.capability : null
            ...
            const liveHolderCapability = liveNameHolder
              ? (stmts.getSessionCapabilityById.get({ $id: liveNameHolder.ctx.sessionId }) as {
                  capability: string | null
                } | null)
              : null
            const fanInHolder =
              ...
              capabilityMatches(
                {
                  capability: liveHolderCapability?.capability ?? null,
                  identity_token: null,
                  launch_id: null,
                  launch_parent_pid: null,
                },
            ...
            if (fanInHolder) {
              const client = applyClient(connId, {
                name: fanInHolder.name,
                role: fanInHolder.role,
                domains: fanInHolder.domains,
                project,
                projectName,
                projectId,
                pid: clientPid,
              ...
              client.connectionEpoch = fanInHolder.connectionEpoch ?? 0
              log.info?.(`capability fan-in: ${resolvedName} member=${client.ctx.sessionId} transport pid=${clientPid}`)
              const coordState = db
                .prepare("SELECT key, value FROM coordination WHERE project_id = ?")
                .all(projectId) as Array<{ key: string; value: string | null }>
              return makeResponse(id, {
            ...
            // deduplicateName rejected every LIVE holder, so any durable row still
            // holding this name is DETACHED. THE ONLY AUTHENTICATOR IS THE
            // SERVER-MINTED CAPABILITY: before a claimant may inherit that row's
            // sessionId (its recipient mailbox + pending balls) OR evict it to
            // squat the name, it must present the row's stored capability. A row
            // with NO stored capability (legacy/unbound) is claimable by name ONLY
            // because deduplicateName already proved there is no live holder — the
            // trust-on-first-use migration path, after which a capability is minted
            // and issued (see below). Name, the public launch tuple, and any
            // derived token are never authority. Auto-generated names (codex1,
            // unknown-x, member-x) are re-minted per spawn, so they are not gated.
            ...
            if (claimRow && !mayClaimDurableRow(claimRow, presentedCapability)) {
              logEvent(daemonCtx, "session.attach_denied", undefined, {
                ...
                stored_capability: claimRow.capability != null,
                presented_capability: presentedCapability != null,
                new_pid: clientPid,
              })
              log.warn?.(
                `identity authorization: denied claim of capability-bound name "${name}" ` +
                  `(stored capability=${claimRow.capability != null}; presented capability=${presentedCapability != null}; new pid=${clientPid})`,
              )
              return makeError(
                id,
                TRIBE_IDENTITY_UNAUTHORIZED,
                `attach to "${name}" denied: the name is bound to a server-minted capability and the presented capability does not match`,
                { reason: "unauthorized-name-claim", name },
              )
            }
            // The capability to persist + issue:
            //   - a capability-BOUND row keeps its stored capability (the claim
            //     already authenticated against it);
            //   - an UNBOUND row's FIRST claimant establishes one: if it PRESENTED
            //     a capability (a supervisor-provisioned launch capability shared
            //     across a managed launch's transports, so siblings can fan in),
            //     store that; otherwise MINT a fresh opaque one (CLI trust-on-
            //     first-use). Either way the capability is random and unforgeable
            //     — never derived from an observable value.
            const issuedCapability = claimRow?.capability ?? presentedCapability ?? mintCapability()
            ...
            // Issue the capability on THIS authenticated connection: a fresh
            // mint for a new/TOFU claim, or the row's existing capability on a
            // legitimate reattach. The client stores it and presents it on every
            // future attach/authority op. It is returned ONLY here — never in any
            // members/status/debug projection.
            return makeResponse(id, {
