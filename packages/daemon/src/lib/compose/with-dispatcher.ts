      ...
      try {
        switch (method) {
          case "register": {
            ...
            // minting a fresh identity and re-announcing a join. deduplicateName
            // above already rejected any LIVE holder, so a hit here is always a
            // detached member — never a collision.
            //
            ...
            //     THIS same row.
            const durableRow = launchIdentity
              ? null
              : (stmts.getDurableSessionByName.get({ $name: name }) as {
                  id: string
                  role: string
      ...
      sock.on("close", () => {
        ...
        if (client && client.role !== "pending") {
          ...
          if (siblingTransport) {
            ...
            // is NOT deleted here (only an explicit `cli_leave` / operator reap
            // removes it), so fetch/pending still resolve the owner and the next
            // same-name connection re-attaches. The detached STATE is derived
            // from liveness (row present, no live connection) in `tribe
            // sessions`. The "left" channel broadcast below stays gated on
