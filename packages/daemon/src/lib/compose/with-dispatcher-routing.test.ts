...
describe("focus-mode notification diet", () => {
  ...
  it.each([
    ...
    expect(shouldDeliver({ kind: "direct", type, topic, replyHint: "yes", settlesOwnRequest: false }, focus)).toBe(
      false,
    )
  })
  ...
  it.each(["request", "query", "assign", "verdict"])("still pushes direct %s actionables", (type) => {
    expect(
      shouldDeliver({ kind: "direct", type, topic: null, replyHint: "yes", settlesOwnRequest: false }, focus),
    ).toBe(true)
  })
  ...
  it("leaves direct notification delivery unchanged until a seat opts in", () => {
    expect(
      shouldDeliver(
        { kind: "direct", type: "github:push", topic: "github:push", replyHint: "yes", settlesOwnRequest: false },
