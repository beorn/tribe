...
describe("25071 stopgap: the hook names a recall that outlived its deadline", () => {
  test("its output carries the skip line, and it exits 0 without waiting for the Worker", async () => {
    ...
    expect(printed[0]).toContain("recall skipped: over the 0.3 s wall (25071)")
    expect(exit).toHaveBeenCalledWith(0)
    expect(JSON.stringify(warned)).toContain('"skipped_steps":{"recall":"recall skipped: over the 0.3 s wall (25071)"}')
  }, 15_000)
})
