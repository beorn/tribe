...
self.onmessage = (event: MessageEvent<{ id: number; query: string }>) => {
  ...
  if (query.startsWith("sqlite:")) {
    ...
    const rows = Math.round(Number(query.slice("sqlite:".length)) * 20_000)
    new Database(":memory:")
      .query(`WITH RECURSIVE c(x) AS (SELECT 1 UNION ALL SELECT x + 1 FROM c WHERE x < ${String(rows)}) SELECT count(*) FROM c`)
      .get()
  } else {
